...
@Component({
  selector: 'app-edit-enc',
  templateUrl: './edit-enc.component.html',
  styleUrls: ['./edit-enc.component.scss']
})
export class EditEncComponent implements OnInit {
  id = 1;
  Forms: FormGroup;
  data_enc_key_update_success 
  ...
  constructor(private snackBar: MatSnackBar,  private cdref: ChangeDetectorRef, private service:MyidService,
    ...
    private models:encmodels,private route: ActivatedRoute, zone:NgZone) { 
      this.data_enc_key_update_success = (event, data) => {
        zone.run(()=>{
      ...
      if ((<any>window).require) {
        try {
          ...
          this.ipc.on('data_enc_key_update_success', this.data_enc_key_update_success)
        } catch (error) {
          console.log(error);
        }
      }
  ...
  ngOnDestroy(){
    this.ipc.removeListener('data_enc_key_update_success',this.data_enc_key_update_success)
  }
  async FormSubmit() {
    ...
    if(keyval.length != 16){
      this.snackBar.open('Wrong key length. Key length must be 16 digits', "Remove", {
        duration: 5000, verticalPosition: 'top', panelClass: ['blue-snackbar']
      });
      return
    }else{
     /*let add =  keyval.charCodeAt(0) + keyval.charCodeAt(1) + keyval.charCodeAt(2)
      ...
      this.ipc.send("update-data-enc",{id:this.id,key:keyval})
